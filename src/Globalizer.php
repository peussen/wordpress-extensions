...
class Globalizer
{
  static protected $contentDir = null;
  ...
  {
    $cacheDir    = self::getContentDir() . '/cache/globalizer/';
  ...
  static public function writeCache($class)
  {
    ...
    $cacheFile   = self::getContentDir() . '/cache/globalizer/' . $classToFile;

    if ( !is_dir(self::getContentDir() . '/cache/globalizer/' . $nsDir )) {
      mkdir(self::getContentDir() . '/cache/globalizer/' . $nsDir,0777,true);
    }
  ...
  static public function getClassStubs($class)
  {
    ...
    $code = "<?php\n// This file was generated by scanglobalize @ " . date('Y-m-d H:m:i') . "\n\n";

    foreach($mapping as $globFun => $method) {
      $original = static::getMethodDocBlock($method);
  ...
  static protected function getMethodDocBlock($method)
  {
    ...
    try {
      ...
      $doc        = preg_replace("/(\n)?\s*\\*/",'$1 *',$doc);
      $doc        = preg_replace('/\*\s+@globalize\s*([a-zA-Z0-9_]+)?.*/m','',$doc);
      return $doc;
  ...
  static private function getContentDir()
  {
    if ( self::$contentDir === null ) {
      if ( defined('WP_CONTENT_DIR')) {
        self::$contentDir = WP_CONTENT_DIR;
      } else {
        self::$contentDir = getcwd() . '/web/app/';
      }
    }
